...
## Verify your simPRO connection

1. Get a bearer token using client credentials (no scopes by default):

   ```bash
   curl -i -X POST \
     -u "$SIMPRO_CLIENT_ID:$SIMPRO_CLIENT_SECRET" \
     -H 'Content-Type: application/x-www-form-urlencoded' \
     -d 'grant_type=client_credentials' \
     "$SIMPRO_TOKEN_URL"
   ```

2. Call the companies endpoint (fallback to alias `0` if your tenant hides the list):

   ```bash
   curl -H "Authorization: Bearer $ACCESS_TOKEN" \
     "$SIMPRO_API_BASE/api/v1.0/companies" \
     || curl -H "Authorization: Bearer $ACCESS_TOKEN" \
          "$SIMPRO_API_BASE/api/v1.0/companies/0"
   ```

3. Start the local stack and open the web app:
