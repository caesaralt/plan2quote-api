...
export const buildQuotePdf = async ({ summary, customerName, projectTitle }) => {
  ...
  doc.moveDown();
  if (env.BRAND_ABN) doc.fontSize(12).text(`ABN: ${env.BRAND_ABN}`);
  ...
  summary.lines.forEach((line, index) => {
    ...
    doc.text(
      `Qty: ${line.quantity} | Unit ex GST: $${line.unitPriceExGst.toFixed(2)} | Total ex GST: $${line.lineTotalExGst.toFixed(2)} | GST: ${(line.gstRate * 100).toFixed(0)}%`
    );
    doc.moveDown(0.5);
  });
